...
function mover(e: EstadoPartida, destino: Celda): Resultado {
  ...
  for (const [i, paso] of ruta.entries()) {
    ...
    // bloque: el monstruo emerge pegado a él y le muerde. El turno sigue y
    // conserva la acción, que es lo que permite devolver el golpe. Lo que le
    // quedaba de movimiento se pierde, como tras cualquier `mover` (regla de
    // la casa de T76, abajo): ya no puede salir corriendo en el mismo turno.
    if (esHeroe(f)) {
